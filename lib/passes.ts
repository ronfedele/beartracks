...
export async function requestPass(params: {
  ...
  const requiresEscort = student.no_roam
  ...
  if (requiresEscort) return { approved: true, passId: pass.id, escort: true, reason: 'Escort Required — see teacher before leaving.' }
