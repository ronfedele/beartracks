            ...
            {!editId ? (
              ...
              <>
                <div className="card space-y-4 border-2 border-bear-orange">
                  ...
                  {(editForm.role === 'teacher' || editForm.role === 'monitor') && (
                    <div className="space-y-3 border-t border-orange-100 pt-3">
                      ...
                      {editForm.role === 'teacher' && (
                        <label className="flex items-center gap-3 cursor-pointer">
                          <input type="checkbox" checked={editForm.same_room_all_day ?? true}
                            onChange={e => setEditForm(f => ({ ...f, same_room_all_day: e.target.checked }))}
                            className="accent-bear-orange w-4 h-4" />
                          <div>
                            <div className="text-sm font-semibold text-bear-dark">Same Room All Day</div>
                            <div className="text-xs text-bear-muted">ON = stays in home room · OFF = can set different rooms per period</div>
                          </div>
                        </label>
                      )}
                    </div>
