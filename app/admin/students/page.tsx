        ...
        {showEdit && (
          <div className="card border-2 border-bear-orange space-y-5">
            ...
            <div className="flex gap-4 items-center">
              <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={form.no_roam} onChange={e=>setForm(f=>({...f,no_roam:e.target.checked}))} className="accent-amber-600"/><span className="text-sm font-medium text-amber-700">🟡 Escort Required</span></label>
                ...
                <tr key={s.id} className="border-b border-orange-50 hover:bg-orange-50/40 transition-colors">
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex gap-1.5">
                      <button onClick={()=>toggleFlag(s.id,'no_roam',!s.no_roam)} title="Escort Required" className={`text-xs px-1.5 py-0.5 rounded-full font-semibold border transition-all ${s.no_roam?'bg-amber-100 text-amber-800 border-amber-200':'bg-gray-50 text-gray-400 border-gray-200 hover:border-amber-200'}`}>🟡</button>
