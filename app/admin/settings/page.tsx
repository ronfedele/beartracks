...
const PRESET_COLORS = [
  { label: 'OMS Orange',  primary: '#E8640A', secondary: '#1a1a2e' },
...
export default function AdminSettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'branding' | 'system' | 'passwords' | 'testing'>('branding')

  // Branding fields (local state, saved all at once)
  const [siteName,      setSiteName]      = useState('')
  const [siteSubtitle,  setSiteSubtitle]  = useState('')
  const [primaryColor,  setPrimaryColor]  = useState('#E8640A')
    ...
    <div className="min-h-screen bg-bear-cream">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex gap-1 bg-orange-100 p-1 rounded-xl w-fit flex-wrap">
          {([['branding','🎨 Branding'],['system','⚙️ System'],['passwords','🔑 Passwords'],['testing','🧪 Testing']] as const).map(([tab, label]) => (
