...
interface RoomLimit {
  id: string
  destination_type: 'bathroom' | 'water' | 'both'
  start_date: string
  end_date: string
  max_passes: number
  note: string | null
...
export default function TeacherPage() {
  ...
  const [soResult, setSoResult] = useState<{ ok: boolean; msg: string; overrideType?: 'escort'|'time'|'limit' } | null>(null)
  const [soLoading, setSoLoading] = useState(false)
  const [pendingOverride, setPendingOverride] = useState<'escort'|'time'|'limit'|null>(null)
  ...
  const loadPeriodStudents = useCallback(async (roomId: string, supabase: any) => {
    try {
      ...
      console.error('loadPeriodStudents error:', err)
      setCurrentStudents([])
    }
  }, [])
  ...
  async function loadLimits(roomId: string) {
    try {
      ...
      // Load per-student usage
      const { data: studs } = await supabase.from('students').select('id').eq('active', true)
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // Students assigned to this room's periods
        const { data: allStuds } = await supabase.from('students').select('*').eq('active', true).order('last_name')
        setAllStudents(allStuds ?? [])

        await loadPeriodStudents(r.id, supabase)
        await loadLimits(r.id)
      } catch (err) {
  ...
  async function handleSignOut(e: React.FormEvent) {
    ...
    const result = await requestPass({
      studentId: soStudentId, roomId: room.id, destinationId: soDestId,
      outBy: profile.email, teacherEmail: room.teacher_email,
      teacherOverride: pendingOverride === 'limit',
      escortOverride:  pendingOverride === 'escort',
      timeOverride:    pendingOverride === 'time',
    })
    if (!result.approved) {
      const r = result as any
      const overrideType: 'escort'|'time'|'limit'|undefined =
        r.escortDenial ? 'escort' : r.timeDenial ? 'time' : r.limitDenial ? 'limit' : undefined
      setSoResult({ ok: false, msg: r.reason, overrideType })
    } else {
      const label = pendingOverride === 'escort' ? '✅ Pass issued — escort required.'
        : pendingOverride ? '✅ Pass issued (teacher override).'
        : '✅ Student signed out.'
      setSoResult({ ok: true, msg: label })
      setSoStudent(''); setSoStudentId(''); setSoMatches([]); setPendingOverride(null)
      await loadLimits(room.id)
    }
    setSoLoading(false)
  }
  ...
  async function saveLimit(destType: 'bathroom' | 'water', form: typeof EMPTY_LIMIT) {
    ...
    const { error } = await supabase.from('room_pass_limits').insert({
      ...
      max_passes: parseInt(form.max_passes), note: form.note || null, active: true,
    })
  ...
  async function removeLimit(limitId: string) {
    const supabase = createClient()
    await supabase.from('room_pass_limits').update({ active: false }).eq('id', limitId)
  ...
  function elapsedClass(min: number) {
    return min < 10 ? 'elapsed-normal' : min < 15 ? 'elapsed-yellow' : min < 25 ? 'elapsed-orange' : 'elapsed-red'
  }
  ...
  return (
    <div className="min-h-screen bg-bear-cream">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* Tabs — Sign Out and Pass Limits only */}
        <div className="flex gap-1 bg-orange-100 p-1 rounded-xl w-fit">
          {([['signout', '✍️ Sign Out'], ['limits', '🚿 Pass Limits']] as const).map(([tab, label]) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {label}
              {tab === 'limits' && limits.length > 0 && (
                <span className="ml-1.5 bg-bear-orange text-white text-xs px-1.5 py-0.5 rounded-full">{limits.length}</span>
              )}
            </button>
          ))}
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="card">
              ...
              <form onSubmit={handleSignOut} className="space-y-4">
                      ...
                      {soMatches.map(s => {
                        ...
                        return (
                          <button key={s.id} type="button"
                            ...
                            {bathroomLimit && u && (
                              <span className={`ml-2 text-xs font-semibold ${u.bathroom >= bathroomLimit.max_passes ? 'text-red-600' : 'text-blue-600'}`}>
                                🚻{u.bathroom}/{bathroomLimit.max_passes}
                              </span>
                            )}
                            {waterLimit && u && (
                              <span className={`ml-2 text-xs font-semibold ${u.water >= waterLimit.max_passes ? 'text-red-600' : 'text-cyan-600'}`}>
                                💧{u.water}/{waterLimit.max_passes}
                              </span>
                            )}
                          </button>
                        )
                ...
                {/* Override prompt — shown when a denial is overridable */}
                {soResult && !soResult.ok && soResult.overrideType && !pendingOverride && (
                  <div className="bg-amber-50 border border-amber-300 rounded-xl px-4 py-3 space-y-2">
                    <p className="text-sm text-amber-800 font-semibold">
                      {soResult.overrideType === 'escort' && '🟡 Escort Required — student cannot leave unaccompanied.'}
                      {soResult.overrideType === 'time'   && '⏰ Time restriction is active for this period.'}
                      {soResult.overrideType === 'limit'  && '🚿 Student has reached their pass limit.'}
                    </p>
                    <p className="text-xs text-amber-700">As the teacher you can override this and issue the pass.</p>
                    <button type="button"
                      onClick={() => { setPendingOverride(soResult.overrideType!); setSoResult(null) }}
                ...
                {pendingOverride && (
                  <div className="bg-amber-50 border border-amber-400 rounded-xl px-4 py-2 flex items-center justify-between">
                    <span className="text-xs font-semibold text-amber-800">
                      🔓 Override Active —&nbsp;
                      {pendingOverride === 'escort' && 'Escort Required bypassed'}
                      {pendingOverride === 'time'   && 'Time restriction bypassed'}
                      {pendingOverride === 'limit'  && 'Pass limit bypassed'}
                    </span>
                    <button type="button" onClick={() => setPendingOverride(null)} className="text-xs text-amber-600 hover:text-amber-800">Cancel</button>
                  </div>
                )}
                ...
                <button type="submit" disabled={soLoading || !soStudentId}
                  className={`w-full font-semibold rounded-xl py-2.5 text-sm transition-colors text-white disabled:opacity-50 ${pendingOverride ? 'bg-amber-600 hover:bg-amber-700' : 'bg-bear-orange hover:bg-orange-600'}`}>
                  {soLoading ? 'Processing…' : pendingOverride ? 'Issue Override Pass' : 'Issue Pass'}
                </button>
              </form>
            </div>
              ...
              {activePasses.length === 0
                ...
                : <div className="space-y-2">
                  {activePasses.map(pass => {
                    ...
                    const ov = (pass.out_by ?? '').startsWith('TEACHER_OVERRIDE:')
                    return (
                      <div key={pass.id} className="flex items-center gap-3 p-3 bg-orange-50 border border-orange-100 rounded-xl">
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-bear-dark text-sm truncate">
                            ...
                            {ov && <span className="ml-1 text-xs text-amber-600">↑ override</span>}
                          </div>
                  ...
                  <div key={destType} className="card space-y-4">
                    ...
                    {existing && (
                      <div className={`border rounded-xl px-4 py-3 space-y-1 ${color}`}>
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-semibold">Max {existing.max_passes} · {existing.start_date} → {existing.end_date}</span>
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-bear-muted mb-1 uppercase tracking-widest">Max Per Student</label>
                        <input type="number" min="1" max="99" value={form.max_passes} onChange={e => setForm(f => ({ ...f, max_passes: e.target.value }))}
                          className="w-full border border-orange-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-bear-orange bg-white" />
                      </div>
                      <div>
        ...
        {/* Roster — shown under sign-out tab */}
        {activeTab === 'signout' && (
          <div className="card">
            <h2 className="text-lg font-bold text-bear-dark mb-3">
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-2">
              {displayStudents.map(s => {
                ...
                const wAt = waterLimit && u.water >= waterLimit.max_passes
                return (
                  <div key={s.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${isOut ? 'bg-amber-50 border border-amber-200' : (bAt || wAt) ? 'bg-red-50 border border-red-100' : 'bg-gray-50 border border-transparent'}`}>
