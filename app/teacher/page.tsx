...
export default function TeacherPage() {
  ...
  const [homeRoom, setHomeRoom] = useState<Room|null>(null)         // default / home room
  const [currentRoom, setCurrentRoom] = useState<Room|null>(null)   // room active this period
  const [allRooms, setAllRooms] = useState<Room[]>([])
  const [periodRooms, setPeriodRooms] = useState<Record<number, Room|null>>({})
  const [sameRoomAllDay, setSameRoomAllDay] = useState(true)
  ...
  const [activeTab, setActiveTab] = useState<'signout'|'schedule'|'limits'>('signout')
  ...
  const loadPeriodStudents = useCallback(async (roomId: string, supabase: any) => {
    try {
      ...
      if (rd.bell_schedule === 9) {
        ...
        if (vc && scheds && scheds.length >= 2) {
          const s7 = scheds.find((s:any) => s.grade_group===7 && s.profile===dayType)
          const s8 = scheds.find((s:any) => s.grade_group===8 && s.profile===dayType)
          if (s7 && s8) {
        ...
        if (sc) periods=[sc.p1_start??sc.day_start,sc.p1_end,sc.p2_start??sc.p1_end,sc.p2_end,sc.p3_start??sc.p2_end,sc.p3_end,sc.p4_start??sc.p3_end,sc.p4_end,sc.p5_start??sc.p4_end,sc.p5_end,sc.p6_start??sc.p5_end,sc.p6_end].map(timeToMin)
      }
      const period = await getCurrentPeriodAsync(periods)
      ...
      setCurrentStudents([])
    } catch(err) { console.error('loadPeriodStudents error:', err); setCurrentStudents([]) }
  }, [])
  ...
  async function loadLimits(roomId: string) {
    try {
      ...
      await Promise.all((studs??[]).map(async (s:any) => {
        ...
        if (bLimit) { const { data:c } = await supabase.rpc('count_student_bathroom_passes',{p_student_id:s.id,p_room_id:roomId,p_start_date:bLimit.start_date,p_end_date:bLimit.end_date}); usage[s.id].bathroom=Number(c??0) }
        if (wLimit) { const { data:c } = await supabase.rpc('count_student_water_passes',{p_student_id:s.id,p_room_id:roomId,p_start_date:wLimit.start_date,p_end_date:wLimit.end_date}); usage[s.id].water=Number(c??0) }
      }))
      setStudentUsage(usage)
    } catch(err) { console.error('loadLimits error:', err) }
  }
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        setSameRoomAllDay(prof.same_room_all_day !== false)

        // Load all rooms for period scheduling
        const { data: rms } = await supabase.from('rooms').select('*').order('room_number')
        setAllRooms(rms??[])

        const home = prof.room as Room|null
        setHomeRoom(home)

        // Load teacher's per-period room assignments
        const { data: tpr } = await supabase.from('teacher_period_rooms').select('*, room:rooms(*)').eq('teacher_id', user.id)
        const prMap: Record<number,Room|null> = {}
        for(let p=1;p<=6;p++) prMap[p] = null
        ;(tpr??[]).forEach((row:any) => { prMap[row.period] = row.room ?? null })
        setPeriodRooms(prMap)

        // Determine active room for current period
        const sameRoom = prof.same_room_all_day !== false
        let activeRoom = home
        if (!sameRoom && tpr && tpr.length > 0) {
          // We'll resolve this after computing the period
          activeRoom = home
        }
        setCurrentRoom(activeRoom)

        if (activeRoom?.id) {
          const { data:allStuds } = await supabase.from('students').select('*').eq('room_id',activeRoom.id).eq('active',true).order('last_name')
          ...
          await loadPeriodStudents(activeRoom.id, supabase)
          await loadLimits(activeRoom.id)
        } else {
          const { data:dests } = await supabase.from('destinations').select('*').eq('active',true).order('sort_order')
          setDestinations(dests??[]); if (dests?.[0]) setSoDestId(dests[0].id)
        }
      } catch(err) { console.error('Teacher init error:', err) }
  ...
  useEffect(() => {
    ...
    async function loadPasses() {
      const { data } = await supabase.from('passes').select('*, student:students(*), destination:destinations(*)').eq('room_id',currentRoom!.id).eq('status','OUT').order('out_time')
      setActivePasses((data??[]) as any)
    }
    loadPasses()
    const ch = supabase.channel('teacher-passes').on('postgres_changes',{event:'*',schema:'public',table:'passes',filter:`room_id=eq.${currentRoom.id}`},loadPasses).subscribe()
  ...
  async function saveScheduleSettings() {
    ...
    const supabase = createClient()
    // Save same_room_all_day flag
    await supabase.from('user_profiles').update({ same_room_all_day: sameRoomAllDay }).eq('id', profile.id)
    // Save per-period rooms
    if (!sameRoomAllDay) {
      await supabase.from('teacher_period_rooms').delete().eq('teacher_id', profile.id)
      const inserts = Object.entries(periodRooms)
        .filter(([,room]) => room?.id)
        .map(([period, room]) => ({ teacher_id: profile.id, period: parseInt(period), room_id: room!.id }))
  ...
  async function handleSignOut(e: React.FormEvent) {
    e.preventDefault(); if (!currentRoom||!profile||!soStudentId||!soDestId) return
    ...
    const result = await requestPass({studentId:soStudentId,roomId:currentRoom.id,destinationId:soDestId,outBy:profile.email,teacherEmail:currentRoom.teacher_email,teacherOverride:isOverride&&isBW})
    if (!result.approved) {
      ...
      if (currentRoom) await loadLimits(currentRoom.id)
    }
    setSoLoading(false)
  }
  ...
  async function saveLimit(destType:'bathroom'|'water', form:typeof EMPTY_LIMIT) {
    ...
    const supabase = createClient()
    await supabase.from('room_pass_limits').update({active:false}).eq('room_id',currentRoom.id).eq('destination_type',destType).eq('active',true)
    const { error } = await supabase.from('room_pass_limits').insert({room_id:currentRoom.id,set_by:profile.id,destination_type:destType,start_date:form.start_date,end_date:form.end_date,max_passes:parseInt(form.max_passes),note:form.note||null,active:true})
    if (error) setLimitMsg(`Error: ${error.message}`)
    else { setLimitMsg(`✅ ${destType==='bathroom'?'Bathroom':'Water fountain'} limit saved.`); await loadLimits(currentRoom.id) }
    setLimitSaving(null)
  }
  ...
  if (!currentRoom) return (
    <div className="min-h-screen bg-bear-cream">
      ...
      <main className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-bear-muted">Your account is not linked to a classroom. Ask your administrator to assign you a room in Admin → Accounts.</p>
        <p className="text-xs text-bear-muted mt-4">{profile?.email}</p>
      </main>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-bear-cream">
      <Nav role={profile?.role??'teacher'} displayName={profile?.display_name??profile?.email} roomNumber={currentRoom?.room_number} />
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        <div>
          <h1 className="text-3xl font-display font-black text-bear-dark">
            {sameRoomAllDay ? `${currentRoom.room_number} · ${currentRoom.teacher_name}` : (currentPeriod ? `Period ${currentPeriod} · ${currentRoom.room_number}` : currentRoom.room_number)}
          </h1>
        ...
        <div className="flex gap-1 bg-orange-100 p-1 rounded-xl w-fit flex-wrap">
          {([['signout','✍️ Sign Out'],['schedule','🗓️ My Schedule'],['limits','🚿 Pass Limits']] as const).map(([tab,label])=>(
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="card">
              ...
              <form onSubmit={handleSignOut} className="space-y-4">
                  ...
                  {soMatches.length>0&&(
                    <div className="absolute z-10 w-full mt-1 bg-white border border-orange-100 rounded-xl shadow-lg overflow-hidden">
                      {soMatches.map(s=>{
                        ...
                        return (<button key={s.id} type="button" onClick={()=>{setSoStudent(`${s.first_name} ${s.last_name}`);setSoStudentId(s.id);setSoMatches([])}}
                ...
                {isOverride&&<div className="bg-amber-50 border border-amber-300 rounded-xl px-4 py-2 flex items-center justify-between"><span className="text-xs font-semibold text-amber-800">🔓 Teacher Override Active</span><button type="button" onClick={()=>setIsOverride(false)} className="text-xs text-amber-600">Cancel</button></div>}
                {soResult&&<div className={`rounded-xl px-4 py-3 text-sm font-medium ${soResult.ok?'bg-green-50 text-green-800 border border-green-200':'bg-red-50 text-red-800 border border-red-200'}`}>{!soResult.ok&&'🚫 '}{soResult.msg}</div>}
              ...
              {activePasses.length===0?<div className="text-center text-bear-muted py-10 text-sm">No students currently out 🎉</div>:
                <div className="space-y-2">{activePasses.map(pass=>{
                  ...
                  return (<div key={pass.id} className="flex items-center gap-3 p-3 bg-orange-50 border border-orange-100 rounded-xl">
                    <div className="flex-1 min-w-0"><div className="font-semibold text-bear-dark text-sm truncate">{pass.student?.preferred_name||pass.student?.first_name} {pass.student?.last_name}{ov&&<span className="ml-1 text-xs text-amber-600">↑ override</span>}</div><div className="text-xs text-bear-muted">{pass.destination?.name}</div></div>
        ...
        {activeTab==='schedule'&&(
          <div className="card space-y-6 max-w-lg">
            <div>
              <h2 className="text-lg font-bold text-bear-dark">My Room Schedule</h2>
              <p className="text-xs text-bear-muted mt-1">Choose whether you stay in one room all day or move between rooms each period.</p>
            </div>

            {/* Same room toggle */}
            <div className="flex items-center gap-4 p-4 bg-orange-50 rounded-xl border border-orange-100">
              <button onClick={()=>setSameRoomAllDay(!sameRoomAllDay)}
                className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${sameRoomAllDay?'bg-bear-orange':'bg-gray-300'}`}>
                <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${sameRoomAllDay?'translate-x-5':'translate-x-0.5'}`} />
              </button>
              <div>
                <div className="text-sm font-semibold text-bear-dark">{sameRoomAllDay ? 'Same room all day' : 'Different room each period'}</div>
                <div className="text-xs text-bear-muted">{sameRoomAllDay ? `You stay in ${homeRoom?.room_number ?? 'your assigned room'} all day` : 'Set a unique room for each period below'}</div>
              </div>
            </div>

            {/* Per-period rooms when not same all day */}
            {!sameRoomAllDay && (
              <div className="space-y-3">
                <div className="text-xs font-semibold text-bear-muted uppercase tracking-widest">Room Per Period</div>
                {[1,2,3,4,5,6].map(p => (
                  <div key={p} className="flex items-center gap-3">
                    <span className="w-16 text-sm font-bold text-bear-dark shrink-0">Period {p}</span>
                    <select value={periodRooms[p]?.id ?? ''}
                      onChange={e => {
                        const room = allRooms.find(r => r.id === e.target.value) ?? null
                        setPeriodRooms(prev => ({ ...prev, [p]: room }))
                      }}
                      ...
                      {allRooms.map(r => <option key={r.id} value={r.id}>{r.room_number} · {r.teacher_name}</option>)}
                    </select>
                  </div>
                ))}
              </div>
            ...
            {schedMsg&&<div className={`text-sm rounded-xl px-4 py-2 ${schedMsg.startsWith('✅')?'bg-green-50 text-green-800 border border-green-200':'bg-red-50 text-red-800 border border-red-200'}`}>{schedMsg}</div>}

            <button onClick={saveScheduleSettings} disabled={schedSaving}
              className="bg-bear-orange hover:bg-orange-600 disabled:opacity-60 text-white font-semibold text-sm px-6 py-2.5 rounded-xl transition-colors">
        ...
        {activeTab==='limits'&&(
          <div className="space-y-6">
              ...
              {(['bathroom','water'] as const).map(destType=>{
                ...
                return (
                  <div key={destType} className="card space-y-4">
                    ...
                    <p className="text-xs text-bear-muted">Max {destType} passes per student over a date range. You can override from your login.</p>
                    {existing&&(<div className={`border rounded-xl px-4 py-3 space-y-1 ${color}`}>
                      <div className="flex items-center justify-between"><span className="text-sm font-semibold">Max {existing.max_passes} · {existing.start_date} → {existing.end_date}</span><button onClick={()=>removeLimit(existing.id)} className="text-xs text-red-500 hover:text-red-700 font-semibold">Remove</button></div>
                      {existing.note&&<div className="text-xs italic opacity-70">{existing.note}</div>}
                    </div>)}
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div><label className="block text-xs font-semibold text-bear-muted mb-1 uppercase tracking-widest">Start</label><input type="date" value={form.start_date} onChange={e=>setForm(f=>({...f,start_date:e.target.value}))} required className="w-full border border-orange-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-bear-orange bg-white" /></div>
                        <div><label className="block text-xs font-semibold text-bear-muted mb-1 uppercase tracking-widest">End</label><input type="date" value={form.end_date} onChange={e=>setForm(f=>({...f,end_date:e.target.value}))} required className="w-full border border-orange-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-bear-orange bg-white" /></div>
                      </div>
                      <div><label className="block text-xs font-semibold text-bear-muted mb-1 uppercase tracking-widest">Max Per Student</label><input type="number" min="1" max="99" value={form.max_passes} onChange={e=>setForm(f=>({...f,max_passes:e.target.value}))} className="w-full border border-orange-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-bear-orange bg-white" /></div>
                      <div><label className="block text-xs font-semibold text-bear-muted mb-1 uppercase tracking-widest">Note (optional)</label><input type="text" value={form.note} onChange={e=>setForm(f=>({...f,note:e.target.value}))} placeholder="e.g. 3 per week" className="w-full border border-orange-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-bear-orange bg-white" /></div>
                      <button onClick={()=>saveLimit(destType,form)} disabled={limitSaving===destType||!form.start_date||!form.end_date}
                        ...
                        {limitSaving===destType?'Saving…':existing?`Update Limit`:`Set Limit`}
                      </button>
                    </div>
                  </div>
                )
            ...
            {(bathroomLimit||waterLimit)&&(
              <div className="card"><h2 className="text-lg font-bold text-bear-dark mb-4">Student Usage</h2>
                <div className="overflow-x-auto"><table className="w-full text-sm">
                  <thead><tr className="border-b border-orange-100">
                    <th className="text-left px-3 py-2 text-xs font-semibold text-bear-muted uppercase">Student</th>
                    {bathroomLimit&&<th className="text-center px-3 py-2 text-xs font-semibold text-blue-600 uppercase">🚻 Bathroom<br/><span className="normal-case font-normal">max {bathroomLimit.max_passes}</span></th>}
                    {waterLimit&&<th className="text-center px-3 py-2 text-xs font-semibold text-cyan-600 uppercase">💧 Water<br/><span className="normal-case font-normal">max {waterLimit.max_passes}</span></th>}
                  </tr></thead>
                  <tbody>{allStudents.map(s=>{
                    ...
                    return (<tr key={s.id} className={`border-b border-orange-50 ${(bAt||wAt)?'bg-red-50':''}`}>
                      <td className="px-3 py-2 font-medium text-bear-dark">{s.preferred_name||s.first_name} {s.last_name}</td>
                      {bathroomLimit&&<td className="px-3 py-2"><div className="flex items-center gap-2 justify-center"><div className="w-20 h-2 bg-gray-100 rounded-full overflow-hidden"><div className={`h-2 rounded-full ${bAt?'bg-red-500':u.bathroom/bathroomLimit.max_passes>0.66?'bg-amber-500':'bg-blue-500'}`} style={{width:`${Math.min(100,(u.bathroom/bathroomLimit.max_passes)*100)}%`}} /></div><span className={`text-xs font-bold ${bAt?'text-red-600':'text-bear-muted'}`}>{u.bathroom}/{bathroomLimit.max_passes}</span></div></td>}
                      {waterLimit&&<td className="px-3 py-2"><div className="flex items-center gap-2 justify-center"><div className="w-20 h-2 bg-gray-100 rounded-full overflow-hidden"><div className={`h-2 rounded-full ${wAt?'bg-red-500':u.water/waterLimit.max_passes>0.66?'bg-amber-500':'bg-cyan-500'}`} style={{width:`${Math.min(100,(u.water/waterLimit.max_passes)*100)}%`}} /></div><span className={`text-xs font-bold ${wAt?'text-red-600':'text-bear-muted'}`}>{u.water}/{waterLimit.max_passes}</span></div></td>}
                    </tr>)
                  })}</tbody>
                </table></div>
              </div>
        ...
        {activeTab==='signout'&&(
          <div className="card">
            ...
            {currentPeriod&&currentStudents.length===0&&<p className="text-xs text-amber-600 mb-3">⚠ No period schedule set — showing homeroom students.</p>}
