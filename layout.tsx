...
interface TerminalState {
  ...
  result: { approved: boolean; reason?: string; passId?: string } | null
}
...
export default function TerminalPage() {
  ...
  async function handleSelectDestination(dest: Destination) {
    ...
    setTimeout(goIdle, result.approved ? 5000 : 7000)
  }
      ...
      <div className="flex flex-1 gap-0">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8 py-10">
          ...
          {state.step === 'result' && state.result && (
            <div className={`text-center animate-fade-in w-full max-w-md ${state.result.approved ? 'approved-flash' : 'denied-shake'}`}>
              {state.result.approved ? (
                <>
                  <div className="text-8xl mb-6">✅</div>
                  <h2 className="text-5xl font-display font-black text-green-400 mb-3">Approved</h2>
                  <p className="text-white/60 text-lg">You are signed out. Return promptly.</p>
                  <p className="text-white/30 text-sm mt-4">This screen will reset in 5 seconds</p>
                </>
              ) : (
                <>
                  <div className="text-8xl mb-6">🚫</div>
                  <h2 className="text-5xl font-display font-black text-red-400 mb-3">Denied</h2>
                  <p className="text-amber-200 text-lg font-medium">{state.result.reason}</p>
                  <p className="text-white/30 text-sm mt-6">This screen will reset in 7 seconds</p>
                </>
              )}
            </div>
          )}
